...
{
// /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/ //
// TODO: Calculate the frequency in Hz based on the value of nu and k.
// Use the pow function
// and supply it with a fractional exponent. Use the reference
// frequency constant,
// k_dFrequencyRef, defined in the header file.
// /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/ //
f = k_dFrequencyRef * pow(2.0, (k / k_nHalfTonesInOctave)) * pow(2.0, nu - 1);
}
else
{
std::cerr << "ERROR: computeFrequency preconditions not satisfied " <<
"dFrequencyHz nu (" << nu << ") or k (" << k << ") invalid" << std::endl;
}
return f;
}
double computeWavelengthInCm(double dFrequencyHz)
{
double dWavelengthCm = -1.0; // Computed wavelength in centimeters returnedby computeWavelengthInCm
// Preconditions:
// dFrequencyHz > 0.0
...
if ((dFrequencyHz < 0.0) || (!std::isfinite(dFrequencyHz)) || (FP_ZERO ==
std::fpclassify(dFrequencyHz)))
{
dWavelengthCm = -1.0; // Indicate Error
std::cerr << "ERROR: computeWavelengthInCm preconditions not satisfied " <<
"dFrequencyHz invalid: " << dFrequencyHz << std::endl;
}
else {
...
//
strNote = a_strNotePrefix[k] + strNu;
// /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/ //
...
std::cout << "Note: " << strNote << "; nu: " << nu << "; k: " <<
k << "; frequency: " << f << " Hz; wavelength: " << dWavelengthCentimeters << " cm"
<< std::endl;
}
}
